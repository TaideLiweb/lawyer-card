...
document.addEventListener('DOMContentLoaded', () => {
    ...
    flatpickr('.popup-content .popup-time .add-time', {
        ...
        onClose: function (selectedDates, dateStr, instance) {
          ...
          <div>${month}/${day}</div>
          <div>${amPm}${hours}:${minutes.length === 1 ? '0' + minutes : minutes}</div>
          <div>5600/1h</div>
        </div>`
            )
            let popupTimeItem = document.querySelectorAll('.popup-content .popup-time .popup-time-item')
    ...
    const serviceMethodTabItem = document.querySelectorAll('.service-method .popup-tab-item')
    const TableTabItem = document.querySelectorAll('.progress-notifiy .popup-tab-item')
    const progressNotifiyTable = document.querySelectorAll('.progress-notifiy .rwd-table')

    const lawKnowledgeTabItem = document.querySelectorAll('.law-knowledge-tab .law-knowledge-tab-item')
    const lawKnowledgeContent = document.querySelectorAll('.law-knowledge-content')

    const preConsultPopup = document.querySelectorAll('.pre-consult')
    const choiceLawyerPopup = document.querySelector('.popup-content.choice-lawyer')
    const noChoiceLawyerPopup = document.querySelector('.popup-content.no-choice-lawyer')
    const inviteNotifyPopup = document.querySelector('.popup-content.invite-notify')
    const matchLawyerBtn = document.querySelectorAll('.match-lawyer-btn')
    const inviteNotifyStatus = document.querySelector('.invite-notify .status>button')
    const inviteNotifyBackIndex = document.querySelector('.invite-notify .back-index>button')
    ...
    // 法律常識 tab
    tabHandle(lawKnowledgeTabItem, lawKnowledgeContent, 'flex')
    ...
    // 預約諮詢的下一步
    document.querySelector('.pre-consult.reserve-consult .consult-btn>button').addEventListener('click', () => {
        ...
        document.querySelector('.choice-lawyer textarea').setAttribute('rows', '4')
        choiceLawyerPopup.style.display = 'flex'
    })

    // 即時諮詢的下一步
    document.querySelector('.pre-consult.timely-consult .consult-btn>button').addEventListener('click', () => {
        serviceMethodTab.style.display = 'none'
        hiddenPopup()
        popupPrevious.style.display = 'block'
        ...
        // 選擇挑選律師出現選律師 popup
        if (document.querySelector('.pre-consult.timely-consult .choice').checked) {
            choiceLawyerPopup.style.display = 'flex'
            // 即時諮詢不給選時間
            // 上一步加上 choice-lawyer-timely
            popupPrevious.classList.add('choice-lawyer')
            document.querySelector('.choice-lawyer textarea').setAttribute('rows', '8')
